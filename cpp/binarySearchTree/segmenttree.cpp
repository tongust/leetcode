...
bool equal_seg_node(seg_node l, seg_node r) {
    return (l.high = r.high) && (l.rang == r.rang);
}
...
vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
    ...
    stree.build_segment_tree(nodes, hs);
    vector<pair<int, int>> sky;
    for (int i = 0; i < N; ++i) {
        auto &bu = buildings[i];
        seg_node sn(bu[0], bu[1], bu[2]);
        for (int k = 0; k < 2; k++) {
            int lookit = bu[k];
            bool notIn = true;
            auto find0_res = stree.search_range(lookit);
            for (auto it : find0_res) {
                if (sn.rang == it->rang && sn.high == it->high) {
                    continue;
                }
                if (it->high >= bu[2]) {
                    if (it->rang.first != lookit && it->rang.second != lookit)
                        notIn = false;
                }
            }
            if (notIn)
                sky.push_back(pair<int, int>(lookit, bu[2]));
        }
    }
    int M = sky.size();
    // duplicate
